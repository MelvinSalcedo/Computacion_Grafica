#ifndef UI_PRIMITIVAS_H
#define UI_PRIMITIVAS_H
#include <windows.h>
#include <GL/glut.h>
#include <stdlib.h>
#include <math.h>
...
void UI_primitivas::put_pixel(int x, int y)
{
	glPointSize(2);
	glBegin(GL_POINTS);
	glVertex2i(x,y);
...
void UI_primitivas::DDA (int x1,int y1,int x2,int y2)
{
...
void UI_primitivas::LSquare(int x1, int y1, int x2, int y2)
{
	DDA(x1,y1,x2,y1);
	DDA(x1,y2,x2,y2);
	DDA(x1,y1,x1,y2);
...
void UI_primitivas::Square(int x1,int y1,int x2,int y2)
{
	int tam;
	bool def_axis;
...
void UI_primitivas::circlePlotPoints (int x1, int y1, int x2, int y2)
{
...
void UI_primitivas::circleMidpoint (int x, int y, GLint radius)
{
	GLint x1, y1;
...
void UI_primitivas::circle(int x1, int y1, int x2, int y2)
{
	GLint r = sqrt(pow(x2-x1,2)+pow(y2-y1,2));
	circleMidpoint(x1,y1,r);
}
void UI_primitivas::PlotPoints(int xc,int yc, int x, int y)
{
	put_pixel(xc+x,yc+y);
	put_pixel(xc-x,yc+y);
	put_pixel(xc+x,yc-y);
...
void UI_primitivas::ElipseMidPoints(int xc, int yc, int rx, int ry)
{
...
void UI_primitivas::Elipse(int x1, int y1, int x2, int y2)
{
	GLint rx = abs(x2-x1);
	GLint ry = abs(y2-y1);
	ElipseMidPoints(x1,y1,rx,ry);
}
void UI_primitivas::Paint(int myFig)
{
	switch(myFig){
...
void UI_primitivas::onMenu(int opcion)
{
	switch(opcion){
...
void UI_primitivas::creacionMenu()
{
...
void UI_primitivas::resize(GLsizei w, GLsizei h)
{
	winWidth = w;
	winHeight = h;
	glViewport(0,0,w,h);
...
void UI_primitivas::display(void)
{
	int i;
	float colores[6][3] = {
		{0.00f, 0.00f, 0.00f},
...
void UI_primitivas::mouse_move(int x, int y)
{
	xt = x;
	yt = winHeight -y;
	glutPostRedisplay();
}
void UI_primitivas::mouse_drag(int x, int y)
{
	xf = x;
	yf = winHeight - y;
	glutPostRedisplay();
...
void UI_primitivas::getPixel(int x, int y, Color rgb)
{
	glReadPixels(x, y, 1, 1, GL_RGB , GL_UNSIGNED_BYTE , rgb);
}
void UI_primitivas::mouse_click(int button , int state , int x, int y)
{
	Color c;
	if(state== GLUT_DOWN){
		xi=x;
...
void UI_primitivas::normal_key(GLubyte key, int x, int y)
{
	char s[80] = "";
...
void UI_primitivas::special_key(int key, int x, int y)
{
	printf("special key # %d\n",key);
}
void UI_primitivas::timer_function(int value)
{
	printf("pointer %d, %d\n",xt,yt);
	glutTimerFunc(8000, timer_function , 1);
}

#endif // UI_PRIMITIVAS_H
